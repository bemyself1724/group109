...
def FF(x, y, z, j):               #布尔函数FF，式中X,Y,Z 为字
    if ((j >= 0) & (j <= 15)):
        ans = Xor3(x, y, z)
    else:
...
def GG(x, y, z, j):                #布尔函数GG，式中X,Y,Z 为字
    if ((j >= 0) & (j <= 15)):
        ans = Xor3(x, y, z)
    else:
