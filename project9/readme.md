...
设 ck_{i,j}  为 CK_i 的第j字节 $(i=0 ,1, \cdots  31; j=0, 1, 2, 3)$, $CK_i−(ck_{i,0}, ck_{i,1}, ck_{i,2}, ck_{i,3}) \in (Z^{8}_2)^4$ , $ ck_{i,j}− (4i + j) \times  7(\ mod \ 256) $ 。其中固定参数 $CK_i(i= 0, 1,\cdots 31)$.
​		
#### **3.** 加密过程
...
### **AES的解密过程**
AES使用SPN结构，加解密函数不同。除了异或密钥，S盒、行移位、列混合都为原先的逆运算。
首先对密文进行一次轮密钥加操作。<br>
